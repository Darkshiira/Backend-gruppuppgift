...
module.exports.PatchCountryHuvudstad = (req, res) => {
    ...
    pool.execute("SELECT * FROM Admins WHERE kod = ?", [kod], (err, results) => {
        ...
        return;
            ...
            const {Namn} = value;
            const {Huvudstad} = value;
            pool.execute('UPDATE Land SET Huvudstad = ? WHERE Namn = ?', [Huvudstad, Namn], (err, results) => {
                if(err) {
                    res.status(500).json(err);
                    return;
                }
                if (results.length == 0) {
                    ...
                    res.status(200).json("Ditt land är uppdaterat med ny huvudstad!");
                }
            })
        }
    });
